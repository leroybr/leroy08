...
const Header: React.FC<HeaderProps> = ({ onNavigate, currentView, onContactHover, showContactForm }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-in-out ${headerBaseClass}`}>
      <div className="w-full px-8 py-3 md:py-4">
        ...
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-baseline gap-16 md:gap-24">
            ...
            <div className="hidden lg:flex gap-12">
              <button onClick={() => onNavigate('real_estate')} className="text-[10px] font-bold tracking-widest uppercase hover:text-leroy-orange transition-all border-b border-transparent hover:border-leroy-orange pb-1">Propiedades en Venta</button>
              <button onClick={() => onNavigate('real_estate')} className="text-[10px] font-bold tracking-widest uppercase hover:text-leroy-orange transition-all border-b border-transparent hover:border-leroy-orange pb-1">Propiedades en Arriendo</button>
            </div>
          </div>
        ...
        {/* Line 1 */}
        <div className={`w-full h-[1px] ${!isHome || scrolled ? 'bg-gray-100' : 'bg-white/20'}`}></div>
